...
while ( (not goal_reached) and (elapsed_time < T_f) ):
    ...
    if min(d) <= d_min:
       ...
       #u_iR[:,:] = dot(HMatrix((sensor_loc[i, :]).T), u_iR[:, :])  # delta_goal^R = dot(H_Si^R, delta_i^Si)
       #Uncomment the above line and put in for loop with increment i w problem
       u_iR[:,:] = dot(HMatrix((sensor_loc[:, :]).T), u_iR[:, i])   # delta_goal^R = dot(H_Si^R, delta_i^Si)
       alpha = atan2(u_iR[:, 1], u_iR[:, 0]) - theta
       u_iR[:, :] = dot(HMatrix(p), u_iR[:, :])  # delta_goal^0 = dot(H_P^0, delta_goal^R)
       temp_goal = zeros(shape=(3, 6))
       temp_goal[:, 0] = p[0]
       temp_goal[:, 1] = p[1]
       dp_bar = u_iR - temp_goal
       rho = norm(dp_bar[0:2])
